...
public class OkHttpClientHelper {
        ...
        if (sOkHttpClient == null) {
            synchronized (HttpMethods.class) {
                if (sOkHttpClient == null) {
                    ...
                    // 指定缓存路径,缓存大小100Mb
                    Cache cache = new Cache(new File(SysApplication.getInstance().getCacheDir(), "HttpCache"),
                            1024 * 1024 * 100);
    ...
    private static Interceptor mRewriteCacheControlInterceptor = new Interceptor() {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            if (!NetUtils.isConnected(SysApplication.getInstance())) {
...
//            LogUtils.d("BODY ===>>>", bodyToString(request));
    ...
    private String getCacheControl() {
        return NetUtils.isConnected(SysApplication.getInstance()) ? CACHE_CONTROL_NETWORK : CACHE_CONTROL_CACHE;
    }
}
